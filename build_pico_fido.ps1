...
foreach ($board in $boards) {
    ...
    $env:PICO_SDK_PATH = if ($env:PICO_SDK_PATH) { $env:PICO_SDK_PATH } else { "../../pico-sdk" }
    cmake .. -DPICO_BOARD=$board
    make -j$($(Get-CimInstance -ClassName Win32_ComputerSystem).NumberOfLogicalProcessors)
    Move-Item pico_fido.uf2 "../release/pico_fido_${board}-${SUFFIX}.uf2"
}
